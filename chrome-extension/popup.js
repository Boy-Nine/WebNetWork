...
function showAuth() { stopPolling(); setStatusPill('idle'); $('authView').classList.remove('hidden'); $('mainView').classList.add('hidden'); $('logoutBtn').classList.add('hidden'); }
function showMain(user) { $('authView').classList.add('hidden'); $('mainView').classList.remove('hidden'); $('logoutBtn').classList.remove('hidden'); setStatusPill('ready'); const phone = user?.phone || ''; $('avatar').textContent = phone.slice(-2) || 'U'; $('helloText').textContent = `Hi，${maskPhone(phone) || 'User'}`; $('userInfo').textContent = `ID ${user?.id || '-'} · 已连接`; syncState(); }
...
$('themeBtn').onclick = toggleTheme; $('authSubmit').onclick = auth; $('startBtn').onclick = startRecording; $('stopUploadBtn').onclick = stopAndUpload; $('discardBtn').onclick = discardRecording; $('webBtn').onclick = () => chrome.tabs.create({ url: WEB });
